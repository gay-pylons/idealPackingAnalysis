...
if __name__ == '__main__':
	...
	for packno in range(numPackings):
		...
		p.load(f'../idealPackingLibrary/{n}/finishedPackings/{name}-{packno}')
#		p.setPhi(p.getPhi()+peakPressure-p.getPressure())
		...
		p.save(f'../idealPackingLibrary/{n}/{posRad}CPPressureSweep/{name}-{packno}/{p.getPressure()}',overwrite=True)
		np.savetxt(f'../idealPackingLibrary/{n}/{posRad}CPPressureSweep/{name}-{packno}/{p.getPressure()}/latticeVectors.dat',lv)
		phiStart = p.getPhi()
